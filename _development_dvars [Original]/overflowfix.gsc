recreateText()
{
	if(self isVerified())
	{
		if(self.menu.open)
		{
			self.menu.tez setSafeText("^5Predator ^4v6.7");
			self.menu.statuss setSafeText("Created by ^5FuSiOn^7 | Access Level: "+verificationToColor(self.status)+"\n^7[{+actionslot 1}] [{+actionslot 2}] scroll | [{+gostand}] select | [{+usereload}] back");
